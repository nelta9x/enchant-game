...
| `ink-soft` | `#6d6650` | 밝은 면 위 보조 글자 — `RecordGauge` 라벨, `SuccessRateSigil` 룬·수치 |
...
| `frame` | `#9c854f` | 금장 테두리 — 보조 버튼 `border-frame/50`, 마법진 `border-frame/25`, 게이지의 "지난 기록" 칸 `bg-frame` |
...
`hit-*`는 캔버스(`HitSparkCanvas`/`hitSparks.ts`) 전용 — 런타임에 1회 rgb로 해석·캐시한다.
...
| `lg:h-[17.25rem]` | 인벤토리 목록(4.5행) | 우측 액션 패널 `lg:h-[23.375rem]`(목록+헤더+골드 합산값) |

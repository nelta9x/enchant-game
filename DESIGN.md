...
| 토큰             | 값          | 의미 · 대표 사용처                                                                         |
| ---------------- | ----------- | ------------------------------------------------------------------------------------------ |
| `bezel`          | `#16181d`   | 근사-검정. 화면 외곽 어두운 프레임 — `GameScreen` 루트 `bg-bezel`; 골드 배지 알약 배경(`GoldDisplay`, 금색 대비) |
...
- **움직이는 것은 transform·opacity 뿐.** 페인트 속성(`clip-path`·`filter`·`box-shadow`·그라데이션 위치·등록 커스텀 프로퍼티)을 프레임마다 바꾸지 않는다 — 한 요소의 페인트 무효화가 루트 레이어 전체(마법진 글로우·점선 원·결계 SVG·8방향 텍스트 섀도)를 다시 래스터한다.
- **`.fx-layer`**(`index.css`): motion/CSS 로 실제로 움직이는 요소에 붙여 자기 합성 레이어를 준다. 대상: 검 등장·떨림 래퍼, 잔상 캔버스, 판매가 pop, 플로팅 텍스트, 코인·아이템·드롭 토큰, 골드창 펄스, 결계 플레어, 최고기록 pop, 강화 버튼(whileTap). 정적 장식엔 붙이지 않는다(레이어 = 텍스처 메모리).
- **재생은 WAAPI(`lib/fx.ts`)**: motion 의 JS 프레임루프 대신 `element.animate()` 로 걸어 컴포지터가 단독 구동한다(연출 중 메인 스레드 프레임 작업 0). 이징 이름은 motion 과 같은 cubic-bezier 로 매핑돼 있다(`EASING_CSS`).
- **쿨다운 리차지**는 디밍 레이어(`.fx-cooldown-dim`)를 `translateX(0→100%)`로 밀어내 밝은 배너를 드러낸다 — 좌→우 채움의 시각은 유지하되 합성만으로 끝난다. 채움 선두의 골드 글로우 띠(0.6rem)는 레이어 왼쪽 가장자리의 그라데이션이다.
...
| 값                                        | 의미                  | 동기화 대상                                                                                           |
| ----------------------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------- |
| `16rem_28rem_16rem` + `gap-2` = **61rem** | 데스크탑 3컬럼 그리드 | `GameScreen`의 그리드·상단(TopControls) 밴드 래퍼 ↔ `CommissionBar` `lg:max-w-[61rem]` — 셋이 한 세트 |
| `lg:h-60` · 세로 `aspect-square w-full`   | 검 박스               | 세로형(`<lg`)은 폭 기준 정사각으로 남는 세로 공간만큼 컬럼 폭까지 커진다(`SwordStage` 마법진 박스). 스프라이트는 세로 `h-2/3 w-2/3`(박스 비례)·`lg:h-40`. 잔상(`ShakeBurstEffect`)도 동일 클래스 |
...
| clamp 분모 `66/48` · 세로 `64`            | 루트 스케일 기준      | 가로 61rem·세로 chrome 실측 높이에서 유도 — 레이아웃 치수 변경 시 재측정 |
| `min(66.67%, calc(100svh-48rem))` (sm: 49.5rem) | 세로 마법진 max-w | 66.67% = 코너 결계 포함 "그룹"(1.5×박스)이 컬럼 폭이 되게 함(겹침 방지). svh 항의 chrome(상단바+거래바+판매가+배너+액션+인벤토리+갭/패딩, 실측 ≈47.5/49rem)은 박스가 남는 높이를 안 넘게 해 스크롤 0. 루트 font-size 와 함께 스케일. 하단 그룹 높이를 바꾸면 N(64)과 함께 재측정 |
...
- 세로형(`<lg`): `min(16px, 100svh/64)` — 화면 높이에 맞춰 줄여 chrome 을 압축한다. 카드는 풀-svh flex
  컬럼이고 마법진 박스(`aspect-square`)가 남는 세로 공간을 채운다 — chrome 이 작아질수록 마법진이
  가져갈 슬랙이 커진다. 박스 폭은 `min(66.67%, calc(100svh−48rem))`(sm: 49.5rem)로 제한돼 항상 남는
  높이에 맞아 **스크롤 0**, 화면이 길면 코너 결계 포함 그룹이 컬럼 폭에서 멈춰 위(마법진)·아래(버튼/인벤토리)로 앵커된다.

...
| `h-16` · 오프셋 `±17%/6%` · `opacity-45` · `ink-soft` 룬 | 검 코너 결계 3종 | `ProtectionWard`(좌상)·`HammerStation`(우상)·`SuccessRateSigil`(우하)가 크기·위치·투명도·색을 공유 — 하나만 바꾸면 셋이 어긋난다(망치 서클이 `opacity-50`이라 더 밝아 보였던 게 그 예). 중앙 마법진(frame)과 달리 코너는 `ink-soft` |

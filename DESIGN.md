...
| `calc(100svh-46rem)`                       | 세로 마법진 max-w     | chrome(상단바+거래바+판매가+배너+액션+인벤토리+갭/패딩) 실측 높이 — 박스가 남는 세로 공간을 넘지 않게 해 스크롤 0(정사각 유지). 루트 font-size 와 함께 스케일. 하단 그룹 높이를 바꾸면 N(64)과 함께 재측정 |
...
- 세로형(`<lg`): `min(16px, 100svh/64)` — 화면 높이에 맞춰 줄여 chrome 을 압축한다. 카드는 풀-svh flex
  ...
  가져갈 슬랙이 커진다. 박스 폭은 `min(컬럼폭, calc(100svh−46rem))`로 제한돼 항상 남는 높이에 맞아
  **스크롤 0**, 화면이 길면 컬럼 폭에서 멈춰 위(마법진)·아래(버튼/인벤토리)로 앵커된다.

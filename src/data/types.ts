...
//      · enhanceParticlesEnabled: 강화 분출 불꽃 캔버스(HitSparkCanvas — 성공=금/파괴=적 색 불꽃을 떨림 끝
//        burstAt 에 폭발). 마운트가 차단되면 등록자가 없어 버스트 emit 은 자동 no-op(particleEmit.ts).
...
//  - particlePoolReserve: 불꽃 객체 풀을 시작(warmup) 시 미리 확보할 슬롯 수. 첫 버스트도 객체를 새로 만들지
//    않게 한다 — 연사 GC 압박을 0으로. 풀은 이보다 큰 버스트가 오면 lazily 더 늘되 줄지는 않는다(상한이 아니라
//    사전 확보치). hitSparks = 한 버스트의 불티+잉걸불 수, hitLicks = 불혀 수. 0 이면 lazily 성장(이전 동작).
//    dots 는 옛 도트 버스트(제거됨)의 잔존 필드 — 더는 쓰이지 않으니 0(스키마 호환 위해 키만 유지).

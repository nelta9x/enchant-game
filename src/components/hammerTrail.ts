...
// 어차피 위에 덮이는 본체(컬러 망치)가 담당한다. 실루엣을 구울 때 1회 베이크한다(hammerSmear —
// 캔버스 엘리먼트의 CSS filter 로 걸면 대형 레이어(46×32rem)가 매 합성마다 blur 를 다시 계산해
// 모바일에서 비싸다). ctx.filter 미지원 브라우저(구형 WebKit)만 CSS filter 폴백으로 돌아간다.

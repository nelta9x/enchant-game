...
// 이번 강화 결과의 떨림 시간(shakeMs, 강화 대상 검의 SwordData.shake[outcome])으로, GameScreen 이
// 효과·사운드·공개·잠금을 거는 데 필요한 마일스톤·수명을 한 번에 도출한다.
//
...
// 버그(crossover 불일치)다. 그래서 강화 1회당 shakeMs 를 1회 정해 이 함수를 1회 부르고, 결과 슬라이스를
...
export type EnhanceTimeline = {
  ...
  shakeMs: number // 이번 강화의 무기 떨림 길이(검 데이터 SwordData.shake[outcome], 0 = 떨림 없음)
...
// shakeMs 는 호출부가 강화 대상 검의 SwordData.shake[outcome] 에서 도출해 넘긴다(0 이하는 0 으로 클램프 →
// 떨림 없음, burstAt = impact). 음수가 들어와 burstAt 이 impact 보다 앞서지 않도록 여기서 한 번 더 막는다.
export function computeEnhanceTimeline(
  anim: AnimationConfig,
  shakeMs: number,
): EnhanceTimeline {
  const impactMs = anim.hammerImpactMs
  // 0 이하 = 떨림 없음 → 0 으로 클램프(음수 burstAt 방지: 떨림이 없으면 임팩트 시점에 바로 버스트·공개).
  const clampedShakeMs = Math.max(0, shakeMs)
  const burstAtMs = impactMs + clampedShakeMs
  return {
    impactMs,
    shakeMs: clampedShakeMs,
    burstAtMs,

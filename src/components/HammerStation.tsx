...
// 같은 회전 마법진(SigilRunes)을 공유해 검을 둘러싼 의식 코너 3종을 완성한다 — 룬 색도 일반 마법진
// 색(ink-soft)으로 맞춰 세 코너가 한 덩어리로 보인다. 다른 두 코너보다 원을 약간 키워(h-20) 가운데
// 망치 스프라이트의 가독성을 확보한다.
//
...
export function HammerStation() {
  return (
    // 위치: 검 우상단 — 좌상단 보호 결계(ProtectionWard)와 좌우 대칭(같은 -17%/6% 오프셋, 좌→우 반전).
    // 검 박스가 overflow-visible 라 박스 밖으로 살짝 나가도 잘리지 않는다(다른 코너 결계와 동일).
    <div
      className="pointer-events-none absolute"
      style={{ right: '-17%', top: '6%' }}
      aria-hidden
    >
      {/* 룬은 일반 마법진 색(ink-soft) — currentColor 로 그려지므로 여기 color 로 물들인다.
          크기·오프셋은 다른 두 코너 결계(h-16, -17%/6%)와 동일하게 맞춘다 — 더 키우면 같은 오프셋에서
          링이 검 안쪽(canvas)으로 파고들기 때문(성공률 결계처럼 검을 건드리지 않게 통일). */}
      <div
        className="relative h-16 w-16"
        style={{ color: 'var(--color-ink-soft)' }}
      >
        {/* 회전 마법진(룬) — 다른 두 코너 결계(ProtectionWard·SuccessRateSigil)와 같은 색(ink-soft)·같은
        ...
        <span className="absolute inset-0 grid place-items-center">
          <img
            ...
            alt=""
            draggable={false}
            className="h-10 w-10 object-contain drop-shadow-[0_4px_8px_rgba(0,0,0,0.35)]"

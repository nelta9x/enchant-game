...
// 미세하게 어긋난다 — "골드와 동일"은 곡선을 공유해야만 보장된다(무엇이 나는지만 다르다).
//
// 안무(코인·아이템 공통): 출발점에서 위로 흩뿌려 "튀어올라"(backOut 바운스) 잠시 체공하다,
// 도착점 반대로 살짝 당겼다(backIn anticipation) 급가속하며 빨려 들어간다. 위치·크기·투명도·회전이
// 같은 곡선을 타 "통 튀어 떠 있다 → 휙 빨려듦"이 일관되게 보인다. (DOTween 의 DOPunchPosition
...
// 체공·회전·방출 지연은 spec(CoinSpec)으로 결정한다(코인은 makeCoins 로 다발, 아이템은 1개 spec).
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        rotate: [0, spec.spin * 90, spec.spin * 150, spec.spin * 520], // 빨려들 때 빛 반사하듯 돈다
      }}
      transition={{
        delay: spec.stagger,

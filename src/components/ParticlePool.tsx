...
import { motion, useAnimationControls, type Transition } from 'motion/react'
...
// 파티클 풀(프레젠테이션 전용) — 성공/실패 버스트와 Hit 불꽃의 모든 파티클을 "고정 개수의 영속 노드를
// 재사용"해 그린다(요구사항: 풀링). 기존엔 버스트마다 motion.span 들을 mount/unmount 했지만, 여기선
// 풀이 앱 수명 동안 N개의 파티클 노드를 항상 띄워 두고(평소 opacity 0·투명), emit() 때 비어 있는 슬롯을
// 골라 색·크기를 입히고 명령형(useAnimationControls)으로 한 번 재생시킨 뒤 다시 idle 로 되돌린다.
// 노드를 새로 만들거나 버리지 않으므로(키=인덱스 고정) 재사용이 성립한다.
//
// 구조:
//  - dot 슬롯(POOL_SIZE개): 사방으로 튀는 작은 파티클(버스트·불티 공용). 데이터에서 최대 단계의 버스트
//    파티클 수 + 불티 수 × 동시성으로 크기를 도출한다(매직 넘버 대신).
//  - decor 슬롯(DECOR_SIZE개): 버스트의 섬광(flash)+충격파 링, 불티의 작은 섬광. 버스트 1회당 1세트라
//    수가 적어 작은 링버퍼로 충분하다.
//  - emit(spec): decor 1세트 + 파티클 N개를 idle 우선으로 배정해 재생한다. 모두 바쁘면 가장 오래된
//    슬롯을 라운드로빈으로 재사용한다(넉넉히 잡아 평소엔 발생하지 않음 — 발생 시 약간의 튐만).
//
// 타이밍은 호출 측(연출 컴포넌트)이 소유한다 — 버스트는 "떨림이 끝난 순간", 불티는 "망치가 닿는 순간"에
// emit() 한다. 그래서 풀은 "지금 이 파티클들을 재생하라"만 알면 되고, delaySec 은 파티클 간 미세 stagger 용이다.
// emit 컨텍스트·훅·타입(ParticleEmitSpec/ParticleEmit)은 particleEmit.ts(비-컴포넌트 모듈)가 소유한다.
...
// 풀 dot 슬롯 수를 데이터에서 도출한다(매직 넘버 금지). 최대 단계 버스트 파티클 + 불티가 한 시도 안에서
// 겹칠 수 있고(불티는 임팩트, 버스트는 떨림 후), 시도끼리도 CONCURRENCY 만큼 겹친다.
function computePoolSize(): number {
  const maxBurst = particleCount(dataManager.getMaxSwordLevel())
  return (maxBurst + HIT_SPARK_COUNT) * CONCURRENCY
}
  ...
  useEffect(() => {
    register(index, {
      ...
      play: (p, spec) => {
        ...
        setStyle({
          bg: `radial-gradient(circle, ${spec.coreVar}, ${spec.edgeVar})`,
          shadow: `0 0 ${spec.kind === 'hit' ? 8 : 6}px ${spec.coreVar}`,
          size: p.size,
        })
        const delay = (spec.delaySec ?? 0) + p.stagger
        // 재사용 직전 상태(옛 재생의 끝값)를 원점으로 즉시 리셋한 뒤 재생 — 항상 검 중심에서 출발한다.
        controls.set({
          x: 0,
          y: 0,
          opacity: 0,
          scale: spec.kind === 'hit' ? 0.5 : 0.4,
        })
        const target =
          spec.kind === 'hit'
            ? {
                // 불티: 위로 솟구쳤다가(정점 p.y, 음수) 중력으로 아래로 떨어진다. 옆으로도 살짝 흘러간다.
                x: [0, p.x, p.x * 1.25],
                y: [0, p.y, Math.abs(p.y) * 0.55],
                opacity: [0, 1, 0],
                scale: [0.5, 1, 0.45],
              }
            : {
                // 버스트: 중심에서 사방으로 뻗어 나가며 페이드(방사형 성공/파괴 분출).
                x: p.x,
                y: p.y,
                opacity: [0, 1, 1, 0],
        ...
        controls.start(target, transition).then(() => {
...
// 섬광(flash) + 충격파 링(영속 노드 1세트) — 버스트마다 1세트 재생. 불티는 작은 섬광만(링 없음).
function DecorSlot({
  index,
  register,
}: {
  ...
  useEffect(() => {
    register(index, {
      ...
      play: (spec) => {
        ...
        const hit = spec.kind === 'hit'
        setFlashBg(
          `radial-gradient(circle, color-mix(in srgb, ${spec.coreVar} 75%, transparent), transparent 70%)`,
        )
        flash.set({ scale: hit ? 0.4 : 0.3, opacity: 0 })
        const flashDone = flash.start(
          {
            scale: hit ? 0.95 : 1.1,
            opacity: [0, hit ? 0.7 : 0.85, 0],
          },
          { delay, duration: hit ? 0.3 : 0.45, ease: 'easeOut' },
        )
        if (hit) {
          // 불티는 링이 없다 — busy 해제는 섬광 완료에 건다.
          flashDone.then(() => {
            if (genRef.current === gen) busyRef.current = false
          })
        } else {
  ...
  return (
    <>
      ...
      <motion.span
        ...
        animate={flash}
      />
      {/* 충격파 링 — 버스트에서만(불티는 링 없음). 색 미지정이면 투명 테두리로 숨는다. */}
      <motion.span
